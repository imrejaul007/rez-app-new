...
const STATUS_STEPS: WebOrderStatus['status'][] = [
  'confirmed',
  'preparing',
  'ready',
  'completed',
...
const STATUS_INFO: Record<string, { label: string; icon: string; color: string; bg: string; msg: string }> = {
  pending_payment: { label: 'Awaiting Payment', icon: 'time-outline',           color: '#D97706', bg: '#FEF3C7', msg: 'Waiting for payment confirmation…' },
  confirmed:       { label: 'Order Confirmed', icon: 'checkmark-circle-outline', color: '#7C3AED', bg: '#F5F3FF', msg: 'Your order has been received by the kitchen!' },
  preparing:       { label: 'Being Prepared',  icon: 'flame-outline',            color: '#2563EB', bg: '#EFF6FF', msg: 'The kitchen is working on your order.' },
  ready:           { label: 'Ready to Serve',  icon: 'bag-check-outline',        color: '#059669', bg: '#ECFDF5', msg: '🎉 Your order is ready! A server will bring it shortly.' },
  completed:       { label: 'Served',           icon: 'happy-outline',            color: '#16A34A', bg: '#F0FDF4', msg: 'Enjoy your meal! Thank you for ordering with REZ.' },
  cancelled:       { label: 'Cancelled',        icon: 'close-circle-outline',     color: '#DC2626', bg: '#FEF2F2', msg: 'This order has been cancelled.' },
};
...
const trackerStyles = StyleSheet.create({
  ...
  dotActive: { width: 28, height: 28, borderRadius: 14, backgroundColor: '#7C3AED', shadowColor: '#7C3AED', shadowOpacity: 0.5, shadowRadius: 6, elevation: 4 },
  line: { position: 'absolute', top: 12, left: '50%', right: '-50%', height: 2, backgroundColor: '#E5E7EB', zIndex: -1 },
        ...
        <Animated.View entering={FadeInDown.delay(280).springify()}>
          ...
          {isTerminal && order.status === 'completed' && (
            <TouchableOpacity
              ...
              onPress={() => router.replace(`/order/${storeSlug}${order.tableNumber ? `?table=${order.tableNumber}` : ''}`)}
            >
...
const styles = StyleSheet.create({
  ...
  liveDot: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, paddingHorizontal: 10, paddingVertical: 5 },
  ...
  sectionTitle: { fontSize: 14, fontWeight: '800', color: '#374151', marginBottom: 14, textTransform: 'uppercase', letterSpacing: 0.4 },

  tableCard: {
    backgroundColor: '#F5F3FF', borderRadius: 12, padding: 12,
    flexDirection: 'row', alignItems: 'center', gap: 8, borderWidth: 1, borderColor: '#DDD6FE',
  },
  tableText: { fontSize: 14, fontWeight: '700', color: '#7C3AED' },

  itemRow: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 6 },
  itemQty: { width: 32, height: 24, backgroundColor: '#F3F4F6', borderRadius: 6, justifyContent: 'center', alignItems: 'center' },
  ...
  readyCTA: {
    ...
    flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderColor: '#A7F3D0',
  },
