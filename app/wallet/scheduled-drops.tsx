      ...
      if (drop.source === 'daily_login') {
        ...
        try {
          ...
          if (result.success && result.data) {
            platformAlertSimple(
              'Check-in Complete',
              `You earned ${(result.data as unknown as Record<string, unknown>).coinsEarned} ${BRAND.CURRENCY_CODE}!`,
            );
            fetchDrops(true);
          } else {
        ...
        try {
          ...
          if (result.success && result.data) {
            platformAlertSimple(
              'Claimed!',
              (result.data as unknown as Record<string, unknown>).message ||
                `You got ${(result.data as unknown as Record<string, unknown>).coins} ${BRAND.CURRENCY_CODE}!`,
            );
            fetchDrops(true);
          } else {
              ...
              filteredDrops.map((drop) => (
                ...
                >
                  <View style={[styles.dropIcon, { backgroundColor: getTypeColor(drop.type) + '20' }]}>
                    <Ionicons
                      name={drop.icon as unknown as keyof typeof Ionicons.glyphMap}
                      size={24}
