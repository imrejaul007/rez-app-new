...
function PharmacyPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (pharmacyResponse.success && (pharmacyResponse.data as unknown as Record<string, unknown>)?.stores) {
        if (!isMounted()) return;
        setPharmacies((pharmacyResponse.data as unknown as Record<string, unknown>).stores);
      }

      // Fetch medicines/products
      const medicineResponse = await apiClient.get('/products?category=healthcare&type=medicine');
      if (medicineResponse.success && (medicineResponse.data as unknown as Record<string, unknown>)?.products) {
        if (!isMounted()) return;
        setMedicines((medicineResponse.data as unknown as Record<string, unknown>).products);
        if (!isMounted()) return;
        setFilteredMedicines((medicineResponse.data as unknown as Record<string, unknown>).products);
      }
    } catch (error: any) {
  ...
  const submitPrescription = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        formData.append('file', { uri: prescriptionImage, name: filename, type } as unknown as Blob);
      }
