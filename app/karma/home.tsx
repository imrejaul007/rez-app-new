      ...
      <View style={styles.eventImageWrap}>
        {event.image ? (
          ...
          <View style={[styles.eventImagePlaceholder, { backgroundColor: catCfg.bg }]}>
            <Ionicons name={catCfg.icon as any as keyof typeof Ionicons.glyphMap} size={32} color={catCfg.color} />
          </View>
        )}
        {/* Category badge */}
  ...
  const fetchData = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        if (profileRes.success && 'data' in profileRes && (profileRes as any as Record<string, unknown>).data) {
          setProfile((profileRes as any as { data: KarmaProfile | null }).data);
